...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    echo "<tr>
            ...
            <td class='action-icons'>
                <a href='update-product.php?batch={$row['batchNumber']}' title='Update'>
                    <i class='bi bi-pencil text-primary'></i>
                </a>
                <a href='../../backend/Pharmacist/product_action.php?action=delete&batch={$row['batchNumber']}' 
                   onclick='return confirm(\"Delete this batch?\")' title='Delete'>
                    <i class='bi bi-trash text-danger'></i>
                </a>
                <a href='dispose-product.php?batch={$row['batchNumber']}' 
