...
if (isset($_GET['action'])) {
    ...
    if ($action == 'delete' && isset($_GET['batch'])) {
        ...
        if (mysqli_query($conn, $query)) {
            header("Location: ../frontend/product-list.php?msg=deleted");
            exit();
        }
    } 
    ...
    elseif ($action == 'update') {
        ...
        header("Location: ../../frontend/Manager/product-list.php?msg=updated");
        exit();
    }

} else {
    // If someone tries to access the file directly without an action
    header("Location: ../../frontend/Manager/product-list.php");
    exit();
}
?>